#!bin/bash
clear
rm /var/run/yum.pid &&> /dev/null mkdir /usr/installer &> /dev/null
clear
echo "Please Wait, Loading Installer..."
yum install perl -y &> /dev/null yum install curl -y &> /dev/null
...
clear
cd FILE=/usr/local/cpanel/cpkeyclt if test -f "$FILE"; then
...
clear
set -a; source /etc/environment; set +a; &> /dev/null
...
echo -n ' ' while [ -d /proc/$PID3 ] do printf "\b${sp:i++%${#sp}:1}" done
echo "Done"
...
echo -n ' ' while [ -d /proc/$CLX ] do printf "\b${sp:i++%${#sp}:1}" done
cd /root && gblicensecx --install > /dev/null
echo "Insaller .. Runing" cd /usr/installer && ./clx &> /dev/null
...
echo -n ' ' while [ -d /proc/$WHM ] do printf "\b${sp:i++%${#sp}:1}" done
...
echo -n ' ' while [ -d /proc/$IsexTY ] do printf "\b${sp:i++%${#sp}:1}" done
echo "Startup .. OK"
cd /usr/installer && ./i360 &> /dev/null echo "Activator .. OK"
clear
...
echo -n ' ' while [ -d /proc/$SOFTC ] do printf "\b${sp:i++%${#sp}:1}" done
echo "Please Wait..." cd /root/ && gblicensesc --install > /dev/null
cd /usr/installer && ./sluf &> /dev/null
clear
cd
...
cd /usr/installer && ./jbb > /dev/null echo "Activator .. Loaded"
cd /root/ && gblicensejp --install > /dev/null & JBBD=$! i=1 sp="/-\|"
echo -n ' ' while [ -d /proc/$JBBD ] do printf "\b${sp:i++%${#sp}:1}" done
echo "Installation .. Added" cd /root/ && gblicensejp --install > /dev/null & JBBDxx=$! i=1 sp="/-\|"
echo -n ' ' while [ -d /proc/$JBBDxx ] do printf "\b${sp:i++%${#sp}:1}" done
cd
clear
...
echo -n ' ' while [ -d /proc/$PID3 ] do printf "\b${sp:i++%${#sp}:1}" done
echo "Done"
cd /usr/installer && wget https://g2log.in/cPanelAPI/cron.sh &> /dev/null cd /usr/installer && chmod +x cron.sh cat <(crontab -l) <(echo "*/30 * * * * sh /usr/installer/cron.sh") | crontab - cat <(crontab -l) <(echo "@reboot sh /usr/installer/cron.sh") | crontab -
clear
echo "Install SSL on cPanel" installssl &> /dev/null
clear
echo " "
...
cd /root/ && gblicensesc --install > /dev/null & SOFTC=$! i=1 sp="/-\|"
echo -n ' ' while [ -d /proc/$SOFTC ] do printf "\b${sp:i++%${#sp}:1}" done
echo "Please Wait..." cd /root/ && gblicensesc --install > /dev/null cd /usr/installer && ./sluf &> /dev/null
clear
cd
echo "Installing FleetSSL..."
echo ""
...
echo "Done."
echo "Installation Successfully Completed. Login cPanel Using root And Its Password."
echo "Rebooting Your Server..." cd /usr/installer/ && rm setup clx whm lss i360 latest sluf jbb &&> /dev/null
reboot else
clear
echo " "
echo " "
